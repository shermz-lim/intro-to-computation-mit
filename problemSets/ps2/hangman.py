...
def hangman(secret_word):
	...
	* After each guess, you should display to the user the 
	...
	while (not is_word_guessed(secret_word, letters_guessed)) and guesses_remaining > 0:
		...
		if (not guess.isalpha()) or (guess in letters_guessed):
			if warnings_remaining > 0:
				...
				if guess.isalpha():
					print("Oops! You have already guessed that letter. You have {} warnings left: {}".format(warnings_remaining, get_guessed_word(secret_word, letters_guessed)))
				else:	
				...
				guesses_remaining -= 1 
				if guess.isalpha():
					print("Oops! You have already guessed that letter. You have no warnings left so you lose one guess: {}".format(get_guessed_word(secret_word, letters_guessed)))
				else:	
	...
	if guesses_remaining > 0:
		total_score = guesses_remaining * len(count_unique_letters(secret_word))	
...
def match_with_gaps(my_word, other_word):
	...
	for i in range(len(stripped_word)):
		if stripped_word[i] == "_":
			continue 
		if not stripped_word[i] == other_word[i]:
			return False 
	return True 		
...
def hangman_with_hints(secret_word):
	...
	* If the guess is the symbol *, print out all words in wordlist that
	...
	while (not is_word_guessed(secret_word, letters_guessed)) and guesses_remaining > 0:
		...
		# Handling case when user guesses * 
		if guess == "*":
